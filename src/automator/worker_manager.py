...
class LockFreeSharedMemoryQueue:
    ...
    def __init__(self, name: str, shape: Tuple[int,...], dtype: np.dtype, capacity: int = 8, create: bool = False):
        ...
        if create:
            ...
            self.buffer_view = self.shm.buf  # FIXED: Defined before index write triggers
            self._write_head(0)
            self._write_tail(0)
        else:
            self.shm = shared_memory.SharedMemory(name=self.name, create=False)
            self.buffer_view = self.shm.buf  # FIXED: Defined uniformly across attachment branches
